...
This paper makes the following contributions to the methodology of strategy evaluation:

\begin{enumerate}
\item \textbf{A conditional null model for timing inference.} The structure-preserving null targets marginal entry-placement performance by holding fixed the realized trade sequence's main structural features. This differs from bootstrap-based or return-shuffling nulls, which preserve return properties but not decision properties.

\item \textbf{An operational definition of entry-placement skill.} Under this framework, skill is not synonymous with profitability. A strategy exhibits entry-placement skill only when its realized timing systematically exceeds what a structurally matched random implementation would generate. This reframes the inferential standard from ``did the strategy make money?'' to ``did this realized entry schedule add information beyond its structural exposure?''

\item \textbf{A new evaluation standard for backtests.} The framework does not replace backtesting but refines its evidentiary meaning. By providing a formal counterfactual, it transforms a descriptive exercise into a statistical test, enabling researchers to isolate the incremental entry-placement component of performance from favorable path dependence, under an explicitly stated schedule measure.

\item \textbf{Synthetic calibration and power evidence, plus a real-data positive control.} The method is evaluated in controlled worlds with no entry skill, volatility clustering, drift-only profitability, structural exposure absorbed by conditioning, and deterministic entry signal. Across $1{,}500$ replications the no-skill worlds reject at the nominal $5\%$ rate and their $p$-values pass a Kolmogorov--Smirnov uniformity test, while power rises monotonically with injected signal. A positive control on the actual GC=F price path closes the loop: when a tunable fraction of entries is placed with genuine foresight, the same structure-preserving null detects it, with the rejection rate rising from a calibrated null baseline to $1.00$ as injected skill increases. A reported non-rejection therefore reflects the strategy, not a mis-sized or powerless test.

\item \textbf{Cross-asset evidence under multiplicity control.} Every completed structure-preserving test on disk---$322$ strategy-by-asset evaluations across $47$ instruments---is pooled and corrected jointly. No result survives Benjamini--Hochberg or Bonferroni control; nominal rejections fall below the chance expectation; and random-entry baselines rank among the most extreme results, demonstrating that nominal significance at breadth reflects luck rather than skill.

\item \textbf{A multi-asset walk-forward external-validity check.} The primary GC=F analysis is supplemented with an exploratory daily walk-forward panel across equity ETFs, single-name equities, futures, commodities, FX, and crypto. This addresses the single-asset criticism while preserving a clear distinction between exploratory breadth and seed-robust inference.

\item \textbf{A bounded frequency-robustness check.} The framework is also run on recent daily, 4-hour, and hourly panels for SPY, QQQ, and BTC-USD. This does not replace a full high-frequency study, but it directly tests whether the main conclusion disappears once the bar interval is shortened.

\item \textbf{A schedule-measure sensitivity and feasibility analysis.} The full panel is re-evaluated under a leading-gap feasibility restriction and a context-matched schedule measure. The verdict is robust to the former but reverses under the latter, demonstrating empirically that timing verdicts are null-relative and establishing agreement across schedule measures---not rejection under any single one---as the standard for a robust timing claim.
\end{enumerate}

\subsection{Related literature and positioning}

The framework addresses a gap between profitability and inference. Classic efficiency perspectives emphasize the difficulty of extracting persistent abnormal performance from price data \cite{fama1970}. Adaptive views allow for episodic opportunities and regime dependence \cite{lo2004}. The data-snooping literature warns that multiple testing and specification search inflate false discovery in strategy research \cite{white2000,hansen2005,sullivan1999}. Recent work on backtest overfitting and research degrees of freedom expands this warning for quantitative pipelines \cite{bailey2017,lopezdeprado2018,baileylopez2014}. Evidence on time-series momentum shows that some directional effects exist in certain asset classes, while robust inference remains difficult \cite{moskowitz2012}.

These approaches address important dimensions of strategy evaluation, yet they answer different questions from the one studied here. White's Reality Check \cite{white2000} and Hansen's SPA test \cite{hansen2005} control for data snooping across a universe of strategies but take each strategy's decision sequence as given. Bootstrap methods \cite{politisromano1994} preserve the statistical properties of returns but randomize the return series rather than the decision process. Bailey et al.\ \cite{bailey2017} quantify the probability of backtest overfitting but operate at the level of strategy selection rather than within-strategy timing. This paper adds a complementary within-strategy diagnostic: whether the realized entry schedule is unusually favorable relative to a randomized timing baseline with the same structural burden.
...
The definition formalizes the central object of the paper. The null distribution captures what the strategy's structural footprint would produce on average under randomized entry placement. Entry-placement skill is the excess of realized return over that distribution. This is a narrow definition by design: it targets one component of strategy performance and tests it against a sharp counterfactual. We do not claim it exhausts the meaning of ``skill,'' only that it is the component for which a clean test is available.

\subsection{Why backtests fail as inferential tools}

Backtests fail as standalone tests of skill for three reasons. First, they provide no counterfactual. A positive return is compared implicitly to zero or to a benchmark, but not to what the same structural burden would have produced under randomized entry placement. Second, backtests are vulnerable to overfitting. With enough specification search, a researcher identifies rules that align with favorable periods in the realized path, producing high returns that reflect path dependence rather than genuine edge \cite{bailey2017}. Third, they conflate market drift with strategy performance. In a trending asset like GC=F over 2002--2026, long-biased exposure can produce positive returns even when entry placement adds little information. The structure-preserving null addresses the first and third problems directly by providing an explicit timing counterfactual on the same realized price path. It does not solve specification search by itself; that requires multiple-testing controls, pre-registration, and out-of-sample replication.
...
Each randomized trade schedule is priced on the observed open-price calendar. Entry and exit reference prices are the randomized opening prices at the simulated entry and exit indices. The simulation reapplies the same execution-cost structure used in the realized trades: a half-spread of 0.5 basis points, a fresh slippage draw uniformly distributed between 0.5 and 3.0 basis points on each side, and an expected commission rate of 0.00002. Together, these assumptions imply the expected round-trip cost of 0.000470 reported in the empirical outputs. If \(\tilde{q}_{j,m}\) denotes the simulated net position return of trade \(j\) in simulation \(m\), then the simulated portfolio return contribution is
\begin{equation}
...
\subsection{Skill metrics computation}

Let \(CR_s^{\mathrm{actual}}\) denote the realized cumulative return of strategy \(s\), and let \(\{CR_{s,m}^{\mathrm{sim}}\}_{m=1}^{M}\) denote the corresponding simulated cumulative returns under the structure-preserving null. Let \(\mu_s^{\mathrm{sim}}\) and \(\sigma_s^{\mathrm{sim}}\) denote the sample mean and standard deviation of \(\{CR_{s,m}^{\mathrm{sim}}\}_{m=1}^{M}\). The reported skill measures are
\begin{equation}
...
\mathrm{RCSI}_{z,s}
=
\frac{CR_s^{\mathrm{actual}}-\mu_s^{\mathrm{sim}}}{\sigma_s^{\mathrm{sim}}}.
\end{equation}
The simulated median is retained as a descriptive reference point, but the final definitions of both \(\mathrm{RCSI}\) and \(\mathrm{RCSI}_z\) are centered on the simulated mean. When the null standard deviation is zero, \(\mathrm{RCSI}_z\) is set to zero only if the realized return equals the null mean and is otherwise treated as undefined.
...
This section places the verified figures with filenames as provided. Captions cite only values printed in the supplied figure captions and tables.
...
\caption{GC=F. Trade-level return ratio by strategy and market regime (daily). The y-axis is mean return divided by standard deviation at the trade level. Light-gray bars are suppressed when regime sample size is fewer than five trades, consistent with the heatmap suppression rule.}
\label{fig:regime_bar}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Figure_10.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Figure_12.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Figure_16.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Figure_17.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{Figure_18.png}
...
The GC=F empirical panel---cumulative returns, trade counts, Monte Carlo $p$-values, percentiles, $\mathrm{RCSI}_z$, regime ratios, and seed robustness in Table~\ref{tab:main}---is transcribed from the verified run outputs Figure\_1.png through Figure\_18.png, together with the supplied Skill vs Luck Summary image (Figure\_19.png), provided with this paper. Those values are not recomputed in this document.

\subsection{Computed analyses and exact commands}

Four analyses are generated directly from the accompanying code rather than transcribed, and reproduce exactly. The synthetic validation (Table~\ref{tab:synthetic_validation}, Figures~\ref{fig:synthetic_calibration} and~\ref{fig:synthetic_power}) is produced, from base seed $9100$, by
\begin{verbatim}
...
\end{verbatim}
which writes \texttt{Data\_Clean/cross\_asset\_panel\_summary.csv}.

\subsection{Statement on empirical integrity}

Every value in the GC=F empirical panel is copied from the supplied figure outputs; every value in the computed analyses above is the direct output of the commands listed in this appendix on the accompanying data. Where a quantity was produced by neither source, the paper states it qualitatively and does not invent a numeric estimate.
